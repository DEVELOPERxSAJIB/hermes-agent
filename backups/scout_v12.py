...
Finds local businesses with verifiable email addresses.
"""
...
LOG_FILE = os.path.join(NANOSOFT_DIR, "scout_v12.log")
STATE_FILE = os.path.join(NANOSOFT_DIR, "scout_v12_state.json")
...
def extract_emails_from_html(html, site_domain):
    """Extract all valid business emails from HTML."""
    found = []
    seen = set()
...
def check_website_playwright(url, scrape_contact_pages=True):
    """Use Playwright to render JS, then scrape for emails + pain points."""
    ...
    with sync_playwright() as p:
        ...
        if scrape_contact_pages:
            for path in ['/contact','/contact-us','/about','/about-us','/team','/staff']:
        ...
        for path, page_url in pages_to_check:
            try:
                ...
                # Extract title (only from homepage)
                ...
                log(f"  Page error {path}: {e}")
                continue
        
        browser.close()
        
        # Extract all emails from all HTML
        ...
        result['emails'] = all_emails[:5]
    
    # Detect pain points from combined HTML
...
def detect_pain_points(html):
    """Detect business pain points beyond just website issues."""
    pains = []
    h = html.lower()[:10000]
    
    # Website issues
    if not re.search(r'<meta[^>]*viewport', h):
        pains.append(("not mobile responsive", "high", "Revenue leakage — 60%+ mobile visitors bounce"))
    if not re.search(r'https:', h.split('<head>')[1][:1000] if '<head>' in h else h[:2000]):
    ...
    if not re.search(r'testimonial|review|google\s+rating|⭐|star\s+rating|fivestar', h):
        pains.append(("no social proof", "medium", "Most buyers read reviews first — they go to competitors who have them"))
    if not re.search(r'<form[^>]*>', h):
        pains.append(("no contact form", "low", "Hard for potential customers to reach you"))
    
    # Operational / business signals
    ...
    if re.search(r'new\s+location|now\s+serving|expanding|grand\s+opening', h):
        pains.append(("expanding business", "opportunity", "Growing = investing in operations"))
    
    # Revenue leakage signals
    if not re.search(r'booking|schedule|appointment|online\s+order|reserve', h):
        pains.append(("no online booking", "high", "Manual booking = lost customers who want instant confirmation"))
    
    if not re.search(r'chat|text|sms|whatsapp|messenger', h):
        pains.append(("no instant communication", "medium", "Competitors with live chat capture your leads"))
...
def search_ddg(niche, city, country="US"):
    ...
    for q in queries:
        ...
        # DDG search: try direct links first (current format), then uddg redirect (old format)
        ...
        skip_domains = {'yelp.com','zocdoc.com','healthgrades.com','vitals.com','webmd.com',
                       ...
                       'avvo.com','findlaw.com','justia.com','superlawyers','martindale'}
        ...
        for i, link in enumerate(links):
            ...
            # Skip aggregators and known non-business sites
            if any(d in domain for d in skip_domains):
                continue
            if domain in seen_domains:
                continue
            # Skip TLDs that aren't business sites
            if any(domain.endswith(t) for t in ['.gov','.edu','.org']) and not domain.endswith('dental.org'):
                continue
...
def scout_niche(niche, city, target=30, country="US"):
    """Main scouting function. Finds local businesses with emails."""
    log(f"\n{'='*60}")
    ...
    for biz in raw_urls:
        ...
        # Must have email — that's the rule
        if not info['emails']:
            no_email += 1
            state.setdefault('seen_domains',[]).append(domain)
            continue
        ...
        # Determine outreach angle from top pain
        if pain_descriptions:
            top = pain_descriptions[0]
            if 'mobile' in top:
                outreach_angle = "Mobile experience losing 60%+ of visitors"
                suggested_solution = "Responsive redesign + mobile-first CTA"
                automation_potential = "high"
            elif 'analytics' in top:
                outreach_angle = "No visibility into what marketing actually works"
                ...
                outreach_angle = "No reviews visible = losing to competitors who have them"
                suggested_solution = "Review display integration + review generation system"
            elif 'hiring' in top:
                outreach_angle = "Growing team = operational scaling needed"
        ...
        log(f"    ✓ {title[:40]} | {email} | {severity} | {pain_str[:50]}")
    ...
    log(f"[SCOUT] Done: {added}/{target} qualified leads")
    log(f"  Checked: {checked} | No email: {no_email} | Parked: {parked} | Dup: {dup}")
